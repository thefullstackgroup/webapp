...
const Tech = ({ user, setSkillsSelected }) => {
  ...
  const sendSlackMessage = async (tech, techExist) => {
    const message = `PROFILE: ${user.name} has ${
      techExist
        ? `removed the tech stack (${tech.label}) from their profile`
        : `added tech stack (${tech.label}) to their profile`
    }`;
